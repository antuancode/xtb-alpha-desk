/**
 * Obtención de datos de mercado y titulares.
 * Antes vivía en src/lib/market.functions.ts y src/lib/news.functions.ts
 * (llamadas RPC desde el navegador); ahora lo consume el motor del servidor.
 * Server-only.
 */
...
export async function fetchMarketData(symbols: string[], interval: string) {
  const cfg = YAHOO_INTERVAL[interval] ?? YAHOO_INTERVAL["15m"];
  ...
  for (const yahooSymbol of symbols) {
    ...
    if (!candles.length && BINANCE_PAIRS[yahooSymbol]) {
      try {
        candles = await fetchBinance(yahooSymbol, interval);
        error = null;
      } catch (e) {
        error = (e as Error).message;
      }
...
export async function fetchNews(tickers: string[]) {
  ...
  return { fetchedAt: Date.now(), items: results.flat().sort((a, b) => b.time - a.time).slice(0, 40) };
}
